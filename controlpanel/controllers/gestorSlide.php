...
class GestorSlide{
	...
	public function mostrarImagenController($datos){
		...
		if($ancho < 1600 || $alto < 600){
			...
			$enviarDatos = array("ruta"=>$respuesta["slide_route"],
				                 "titulo"=>$respuesta["slide_title"],
				                 "descripcion"=>$respuesta["slide_description"]);
	...
	public function mostrarImagenVistaController(){
		...
		foreach($respuesta as $row => $item){
			echo '<li id="'.$item["slide_id"].'" class="bloqueSlide">
					<span class="fa fa-times eliminarSlide" ruta="'.$item["slide_route"].'">
					</span><img src="'.substr($item["slide_route"], 6).'" class="handleImg">
				  </li>';
		}
	}
	...
	public function editorSlideController(){
		...
		foreach($respuesta as $row => $item){
			echo '<li id="item'.$item["slide_id"].'">
					<span class="fa fa-pencil editarSlide" style="background:blue"></span>
					<img src="'.substr($item["slide_route"], 6).'" style="float:left; margin-bottom:10px" width="80%">
	...
	public function actualizarSlideController($datos){
		...
		$enviarDatos = array("titulo"=>$respuesta["slide_title"],
			                 "descripcion"=>$respuesta["slide_description"]);
